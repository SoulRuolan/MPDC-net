...
parser.add_argument('--root_path', type=str, default='/root/autodl-tmp/Database/PROMISE12/h5py/test/',
...
def test_single_volume(case, net, test_save_path, FLAGS):
    ...
    for ind in range(image.shape[0]):
        ...
        with torch.no_grad():
            ...
            # out = torch.argmax((torch.softmax(-1 * out_main, dim=1)), dim=1).squeeze(0) # Reverse output
...
def Inference(FLAGS):
    with open('../data/Prostate/test.list', 'r') as f:
        image_list = f.readlines()
    image_list = sorted([item.replace('\n', '').split(".")[0] for item in image_list])

    snapshot_path = "../model/Prostate".format(FLAGS.model, FLAGS.exp, FLAGS.labeled_num)
    test_save_path = "../model/Prostate/predictions/".format(FLAGS.model, FLAGS.exp, FLAGS.labeled_num)
    ...
    for case in image_list:
        ...
        logging.info("%s : " % (case))
        logging.info('%s : dice : %f  jc : %f  hd95 : %f  asd : %f' % (
        "avg_metric", single_avg_metric[0], single_avg_metric[1], single_avg_metric[2], single_avg_metric[3]))
        logging.info('%s : dice : %f  jc : %f  hd95 : %f  asd : %f' % (
        "first_metric", first_metric[0], first_metric[1], first_metric[2], first_metric[3]))
        logging.info('%s : dice : %f  jc : %f  hd95 : %f  asd : %f' % (
        "second_metric", second_metric[0], second_metric[1], second_metric[2], second_metric[3]))
        logging.info('%s : dice : %f  jc : %f  hd95 : %f  asd : %f' % (
        "third_metric", third_metric[0], third_metric[1], third_metric[2], third_metric[3]))
