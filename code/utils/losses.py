...
class DiceLoss_ra(nn.Module):
    def __init__(self, n_classes):
        super(DiceLoss_ra, self).__init__()
        self.n_classes = n_classes
...
class Cos(nn.Module):
    ...
    def _ce_loss(self, score, target):
        ...
        loss = (1 + cos_dis(target, score)) / 2
        return loss

    def forward(self, inputs, target):
        inputs = -1 * (torch.softmax(inputs, dim=1)) + 1
        target = self._one_hot_encoder(target)
        loss = 0.0
        for i in range(0, self.class_num):
...
class Cos_feat(nn.Module):
    def __init__(self, n_classes):
        super(Cos_feat, self).__init__()
        self.class_num = n_classes
    ...
    def _ce_loss(self, score, target):
        ...
        loss = 0.0
        loss += (1 + cos_dis(score.detach(), target)) / 2
        loss += (1 + cos_dis(target.detach(), score)) / 2
    ...
    def forward(self, inputs, target):        
        loss = 0.0
        loss += self._ce_loss(inputs, target)   
        return loss / self.class_num
...
class KL(nn.Module):
    ...
    def _ce_loss(self, score, target):
        loss = F.kl_div(torch.log(F.softmax(target)).detach(), F.softmax(score), reduction='mean')
        return loss
